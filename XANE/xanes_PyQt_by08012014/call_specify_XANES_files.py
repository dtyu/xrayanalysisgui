...
class MyForm(QtGui.QDialog):
  def __init__(self, parent=None):
    ...
    self.ui.fitting_method.addItems(['select one:','brute force','RF norm','RF'])
    ############################
    self.scale_max = 1.00
    self.scale_min = 0.00
  ...
  def eventFilter(self, source, event):
	...
	if (event.type() == QtCore.QEvent.MouseButtonPress and source is self.ui.fittingView and self.selectpointPress == True):
	    ...
	    self.fitImagePosY = self.fitImagePos.y()
            print('fitImagePos: (%d, %d)' % (self.fitImagePosX/329*1024, self.fitImagePosY/329*1024))
	    self.ui.x_cord.setText(str(int(self.fitImagePosX/329*1024)))
            self.ui.y_cord.setText(str(int(self.fitImagePosY/329*1024)))
            self.selectpointPress = False
	    QtGui.QApplication.restoreOverrideCursor()
  ...
  def StartFit(self):
      ...
      if str(self.ui.fitting_method.currentText()) == 'brute force':
          if self.ref3 == None:
             self.min_R, self.index, self.fRGB = fit2D_2ref(self.ref1[start-1:end],self.ref2[start-1:end],self.norm_image_stack[:,:,start-1:end],self.reso)
          else:
             self.min_R, self.index, self.fRGB = fit2D_3ref(self.ref1[start-1:end],self.ref2[start-1:end],self.ref3[start-1:end],self.norm_image_stack[:,:,start-1:end],self.reso)
