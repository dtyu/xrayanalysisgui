...
import numpy as np
import subprocess
from fitting import fit2D_2ref, fit2D_3ref
...
class MyForm(QtGui.QDialog):
  ...
  def ShowPlot(self,x,preStart=None,preEnd=None,postStart=None,postEnd=None,edgeStart=None,edgeEnd=None,prePress=None,postPress=None,edgePress=None,fitStart=None,fitEnd=None):
      ...
      if "Raw Spectrum (ROI) 'cyan *'" in x:
	      ...
	      self.raw_roi = np.mean(np.mean(data, axis=0), axis=0)
              ...
              self.raw_roi = -np.log(self.scale_image_stack[X,Y,0:self.iev])
	      self.ax.plot(self.ev, self.raw_roi, '*-', color="c")
  ...
  def StartNormalize(self):
	  ...
	  data = self.log_image_stack 
          ...
          if self.defaultState == 1:
              scale_0 = np.mean(self.log_image_stack[:,:,0:4], axis=None)
              scale_1 = np.mean(self.log_image_stack[:,:,self.iev-5:self.iev-1], axis=None)
          else:
              scale_0 = np.mean(self.log_image_stack[:,:,self.preStart-1:self.preEnd-1], axis=None)
              scale_1 = np.mean(self.log_image_stack[:,:,self.postStart-1:self.postEnd-1], axis=None)
          print(scale_0)
  ...
  def SaveSpectrum(self):
	  ...
	  wildcard =  "TXT (*.txt)" 
          ...
          f.write('    Energy:               mut:\n')
          for i in range(self.iev):
              f.write("%14.5f%14.4f\n".format(self.ev[i],self.raw_bulk[i]))
          f.close()
	  ...
	  wildcard =  "TXT (*.txt)" 
          ...
          f.write('    Energy:               mut:\n')
          for i in range(self.iev):
              f.write("%14.5f%14.4f\n".format(self.ev[i],self.norm_bulk[i]))
          f.close()
	  ...
	  wildcard =  "TXT (*.txt)" 
          ...
          f.write('    Energy:               mut:\n')
          for i in range(self.iev):
              f.write("%14.5f%14.4f\n".format(self.ev[i],self.norm_roi[i]))
          f.close()
  ...
  def GetRmax(self):
      self.rmax = int(self.ui.Rmax.text())
  ...
  def StartFit(self):
      ...
      if self.ui.bin_object.isChecked:
          a =0
          #weight = np.repeat(1.0,self.bin)/self.bin
          #for i in range(self.iev):
              #self.log_image_stack[:,:,i] = np.convolve(self.log_image_stack[:,:,i],weight,'valid')
      if self.ui.smooth_object.isChecked:
          weight = np.repeat(1.0,self.smooth)/self.smooth
          for i in range(self.n_rows):
	      for j in range(self.n_cols):
                  self.log_image_stack[i,j,:] = np.convolve(self.log_image_stack[i,j,:],weight,'valid')
      if self.ref3 == None:
         self.min_R, self.index, self.fRGB = fit2D_2ref(self.ref1[a-1:b],self.ref2[a-1:b],self.log_image_stack[:,:,a-1:b],self.reso)
      else:
         self.min_R, self.index, self.fRGB = fit2D_3ref(self.ref1[a-1:b],self.ref2[a-1:b],self.ref3[a-1:b],self.log_image_stack[:,:,a-1:b],self.reso)
      ...
      #self.ui.athena.setEnabled(True)

  def ShowFittedImage(index,fRGB,rmax,filter_image,min_R):
      fittedresult = np.array((self.n_rows,self.n_cols,3), dtype=int)
      for i in range(self.n_rows):
	  for j in range(self.n_cols):
              fittedresult[i,j,:] = fRGB[index[i,j]]
      fittedresult[filter_image == 0] = [0,0,0]
      fittedresult[min_R >= rmax] = [0,0,0]
      image = qimage2ndarray.numpy2qimage(np.array(255*fittedresult, dtype=int))
      pixmap = QtGui.QPixmap.fromImage(image)
  ...
  def UpdateImage(self):
      self.ShowFittedImage(self.index,self.fRGB,self.rmax,self.filter_image)
