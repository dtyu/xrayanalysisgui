...
handles.filter = ones(handles.imgHeight,handles.imgHeight);
handles.MaxRfilter = ones(handles.imgHeight,handles.imgHeight);
...
if ismember(2, method_index_selected)
    ...
    set(handles.DefaultScaling, 'Value', 1)
    set(handles.PreEstart, 'Enable', 'off')
    set(handles.PreEend, 'Enable', 'off')
    set(handles.PostEstart, 'Enable', 'off')
    set(handles.PostEend, 'Enable', 'off')
...
if handles.Raw_Bulk_Spectrum  
    ...
    point_spectrum = squeezed_ln_imgEstack;
...
if handles.ref1plot
    if isempty(handles.ref1)
        msgbox('Please Input Reference 1 First!')
        return
    end
    axes(handles.axes2);
...
if handles.ref2plot
    if isempty(handles.ref2)
        msgbox('Please Input Reference 2 First!')
        return
    end
    axes(handles.axes2);
...
if handles.ref3plot
    if isempty(handles.ref3)
        msgbox('Please Input Reference 3 First!')
        return
    end
    axes(handles.axes2);
...
handles.RmaxValue = str2double(get(hObject,'String'))/100;
...
cd(path)
handles.image = zeros(size(handles.index),3);
for i=1:handles.imgHeight
    for j=1:handles.imgHeight
        for k=1:3
            handles.image(i,j,k) = handles.fRGB(handles.index(i,j),k);
        end
    end
end
handles.filter3d = repmat(handles.filter,[1,1,3]);
handles.MaxRfilter3d = repmat(handles.MaxRfilter,[1,1,3]);
handles.image = handles.image.*handles.filter3d.*handles.MaxRfilter3d;
% imwrite(handles.index.*handles.filter.*handles.MaxRfilter, handles.fRGB, [path, file]);
imwrite(handles.image, [path, file]);
