...
#----------------------------------------------------------------------    
    def read_xrm_list(self, filename, ds): 
        
        #Fill the common stack data 
        #file1 = os.path.join(filepath, filelist[0])   


        if not isOleFile(filename[0]):
            print "File not valid OLE type."
            return
        # Open OLE file:
        ole = OleFileIO(filename[0])
        ...
        if ole.exists('ImageInfo/ImageHeight'):                  
            ...
            nev = struct.unpack('<I', data)
            if verbose: print "ImageInfo/ImageHeight = %i" % nev[0]  
            self.n_rows = np.int(nev[0])
            
           

        npix = self.n_cols*self.n_rows
        self.n_ev = len(filename)
        ...
        for j in range(len(filename)):
            ...
            self.absdata[:,:,j] = np.reshape(imgdata, (self.n_cols, self.n_rows), order='F')
            self.ev[j] = eng[0]
            ...
            #Energy saved in the file is too coarse - read it from the filename
            filename_list = filename[j]
            str_list = filename_list[:-4].split('_')
            #try: #look for 'eV' filed in the filename
                #for i in range(len(str_list)):
                    ## In this case energy is stored like this: _0250.52eV_
                    #if 'eV' in str_list[i]:
        ...
        x_dist = np.arange(np.float(self.n_cols))*pixelsize
        y_dist = np.arange(np.float(self.n_rows))*pixelsize
