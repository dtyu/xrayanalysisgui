...
function [TXRMinfo, TXRM_img] = loadXANESMultiXRMv2(XRMdir, XRMprefix, XRMEi, XRMEstep, XRMEf, XRMending)
%noted that the Ei, Estep and Ef are all string format here...

%this is the synthax for using freadss; you may edit the fields and
%relating data type for your own interests

%use the user input as the energy
TXRMinfo.Energy = [str2double(XRMEi):str2double(XRMEstep):str2double(XRMEf)];
TXRMinfo.ImagesTaken = (str2double(XRMEf)- str2double(XRMEi))./str2double(XRMEstep)+1;

%use the first file's information as the overall information, except for
%energy and image contents

XRMfilename = [XRMdir, XRMprefix, XRMEi, XRMending, '.xrm']
[info,err,errmsg] = freadss(XRMfilename, ...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
i = 1
for Enow = str2double(XRMEi):str2double(XRMEstep):str2double(XRMEf)
    XRMfilename = [XRMdir, XRMprefix, num2str(Enow), XRMending, '.xrm']
    [img,err,errmsg] = freadss(XRMfilename, ...
        {['ImageData1\Image1']}, ...
        {'uint16'});
